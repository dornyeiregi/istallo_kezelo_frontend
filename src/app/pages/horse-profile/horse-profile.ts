...
export class HorseProfilePage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const navHorse = this.router.getCurrentNavigation()?.extras.state?.['horse'] as
      | HorseDTO
      | undefined;
    if (navHorse) {
      this.horse = navHorse;
  ...
  loadFeedScheds(horseId: number): void {
    ...
    forkJoin({
      ...
      next: ({ feedScheds, items, horses, pending }) => {
        this.feedScheds = feedScheds.sort((a, b) => (b.feedSchedId || 0) - (a.feedSchedId || 0));
        this.items = items;
  ...
  submitVaccination() {
    ...
    if (this.vaccinationOption === 'existing') {
      ...
      this.horseShotService.addShotToHorse(this.selectedShotId, this.horse.id).subscribe(() => {
  ...
  submitTreatment() {
    ...
    if (this.treatmentOption === 'existing') {
      ...
      const selected = this.allTreatments.find((f) => f.treatmentId === this.selectedTreatmentId);
      ...
      this.horseTreatmentService.create(dto).subscribe(() => {
  ...
  submitFarrierApp() {
    ...
    if (this.farrierOption === 'existing') {
      ...
      this.horseFarrierAppService.create(dto).subscribe(() => {
  ...
  submitFeedEditor() {
    ...
    const hasAnyTime =
      this.feedEditor.morning.enabled ||
      this.feedEditor.noon.enabled ||
      this.feedEditor.evening.enabled;
    if (!hasAnyTime) {
      this.horseFeedSchedService.deleteAllForHorse(this.horse.id).subscribe(() => {
  ...
  private getExistingFeedSchedsByTime(): Record<FeedTimeKey, FeedSchedDTO | null> {
    ...
    const withHorse = this.feedScheds.filter(
      (feed) => this.currentHorseId != null && feed.horseIds?.includes(this.currentHorseId),
    );
